...
export const calcStrikeRate = (runs, balls) => {
 if(balls<=0 || runs<0) return 0;
  ...
  return Number(strikeRate.toFixed
    (2));
};
...
let battingAvg=totalRuns/(innings-notOuts);
 return Number(battingAvg.toFixed
    (2));
};
...
export const getPlayerCard = (player) => {
  ...
  if(!player|| typeof player.name!=="string"||player.name.trim().length==0) return null;
    const { name,runs,balls, totalRuns, innings,notOuts = 0,runsConceded,overs} = player;
  ...
  let allRounderStatus=isAllRounder(battingAvg,economy)

  return {name,strikeRate,economy,battingAvg,isAllRounder:allRounderStatus }
};
